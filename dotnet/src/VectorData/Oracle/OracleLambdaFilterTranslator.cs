...
internal sealed class OracleLambdaFilterTranslator : OracleSqlFilterTranslator
{
    internal List<OracleParameter> _oracleParameterList = [];
    internal OracleSKMetadata _oracleSKMetadata;

    private readonly CollectionModel _model;
    private readonly LambdaExpression _lambdaExpression;
    private readonly ParameterExpression _recordParameter;
    internal readonly StringBuilder _sql;

    public OracleLambdaFilterTranslator(
    ...
    internal void Translate(Expression? node, bool isSearchCondition = false)
    {
        switch (node)
        {
    ...
    internal void TranslateBinary(BinaryExpression binary)
    {
        ...
        string colName;
        this._sql.Append('(');
        if (binary.Left is ConstantExpression && binary.Right is MemberExpression)
        { // ex: 8 == r.id
            this.TranslateQueryParameter(((ConstantExpression)binary.Left).Value, out oracleParameter);
            AppendBoolOperator(binary);
            colName = this.TranslateMember((MemberExpression)binary.Right, isSearchCondition: binary.NodeType is ExpressionType.AndAlso or ExpressionType.OrElse);
            oracleParameter.OracleDbType = this._oracleSKMetadata.AllColumnsByDbObjName[colName].OraDbType;
        }
        else if (binary.Left is MemberExpression && binary.Right is ConstantExpression)
        { // ex: r.id == 8
            colName = this.TranslateMember((MemberExpression)binary.Left, isSearchCondition: binary.NodeType is ExpressionType.AndAlso or ExpressionType.OrElse);
            AppendBoolOperator(binary);
            this.TranslateQueryParameter(((ConstantExpression)binary.Right).Value, out oracleParameter);
            oracleParameter.OracleDbType = this._oracleSKMetadata.AllColumnsByDbObjName[colName].OraDbType;
        }
        else
        { // all other cases
        ...
        void AppendBoolOperator(BinaryExpression binary)
    ...
    internal void TranslateConstant(object? value)
    {
        // TODO: Nullable
        switch (value)
        {
    ...
    private string TranslateMember(MemberExpression memberExpression, bool isSearchCondition)
    {
        if (this.TryBindProperty(memberExpression, out var property))
        {
            return this.GenerateColumn(property, isSearchCondition);
        }
    ...
    internal string GenerateColumn(PropertyModel property, bool isSearchCondition = false)
    {
        // StorageName is considered to be a safe input, we quote and escape it mostly to produce valid SQL.

        // TODO: Please check if mapping is correct.
        string columnName = this._oracleSKMetadata.DataPropertyNameToDbObjNameMappings[property.ModelName];
    ...
    internal void TranslateContainsOverArrayColumn(Expression source, Expression item)
    {
        throw new NotSupportedException();
    }

    internal void TranslateContainsOverParameterizedArray(Expression source, Expression item, object? value)
    {
        throw new NotSupportedException();
    }

    internal void TranslateQueryParameter(object? value)
    {
        this._sql.Append(':').Append(this._oracleParameterList.Count);
        this._oracleParameterList.Add(new OracleParameter("", value));
    }

    internal void TranslateQueryParameter(object? value, out OracleParameter oracleParameter)
    {
        this._sql.Append(':').Append(this._oracleParameterList.Count);
        oracleParameter = new OracleParameter("", value);
        this._oracleParameterList.Add(oracleParameter);
    }
    // Enable for parameterized SQL
    /*
    internal override void TranslateConstant(object? value)
    {
        this.TranslateQueryParameter(value);
    }
    */

    internal override OracleSqlCommandInfo GenerateSQL(OracleDataModelMetadata metadata, int startIndex)
    {
        this._oracleSKMetadata = (OracleSKMetadata)metadata;
        this.Translate(true);
        //TODO: use startIndex for _oracleParameterList
        OracleSqlCommandInfo cmdInfo = new()
        {
            SqlText = this._sql.ToString(),
            Parameters = this._oracleParameterList,
        };
        return cmdInfo;
    }
}
